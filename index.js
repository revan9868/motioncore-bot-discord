  ...
  try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === COMMAND_NAME) {
      ...
      const container = new ContainerBuilder()
        ...
        .addTextDisplayComponents(
          new TextDisplayBuilder().setContent([
            '**💎 VIP Script**',
            'Akses penuh • Update prioritas • Support 24/7',
            '',
            '**⌛ QRIS**',
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('modal_order_')) {
      ...
      try {
        ...
        const qrisContent = new ContainerBuilder()
          .setAccentColor(0x7803eb)
          .addTextDisplayComponents(
            new TextDisplayBuilder().setContent([
              `### ${packageInfo.label}`,
          ...
          .addFileComponents(new FileBuilder().setURL('attachment://qris.png'));
        ...
        try {
          ...
          await qrUser.send({ components: [qrisContent], files: [attachment] });
